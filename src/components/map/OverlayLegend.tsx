...
export default function OverlayLegend({ type }: OverlayLegendProps) {
  if (type === "airspace") {
    return (
      <div className="absolute bottom-24 right-4 z-[900] bg-card/95 backdrop-blur rounded-xl border border-border shadow-xl p-3 w-44">
        <p className="text-xs font-semibold text-foreground mb-2">Airspace Classes</p>
        <div className="flex flex-col gap-1.5">
          {AIRSPACE_CLASSES.map((c) => (
            <div key={c.label} className="flex items-center gap-2">
              <div className="w-3.5 h-3.5 rounded-sm flex-shrink-0" style={{ backgroundColor: c.color }} />
