...
export default function LayerSwitcher({ activeLayer, onChange }: LayerSwitcherProps) {
  ...
  return (
    <div className="absolute bottom-14 right-3 z-[900]">
      {open && (
        <div className="mb-2 bg-card/95 backdrop-blur rounded-xl border border-border shadow-xl p-1.5 grid grid-cols-2 gap-1 w-36">
          {LAYERS.map((layer) => (
            <button
              key={layer.id}
            ...
            >
              <span className="text-base">{layer.preview}</span>
              {layer.label}
            </button>
          ))}
        </div>
      ...
      <button
        ...
        className={`w-8 h-8 rounded-lg flex items-center justify-center shadow-lg border transition-all ${
