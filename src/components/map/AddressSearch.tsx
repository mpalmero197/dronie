...
export default function AddressSearch() {
  ...
  return (
    <div
      ref={containerRef}
      className="absolute top-3 left-1/2 -translate-x-1/2 z-[950]"
      onMouseDown={(e) => e.stopPropagation()}
    >
      {!expanded ? (
        <button
          onClick={() => { setExpanded(true); setTimeout(() => inputRef.current?.focus(), 100); }}
          className="w-9 h-9 rounded-lg bg-card/95 backdrop-blur border border-border shadow-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-secondary transition-all"
        >
